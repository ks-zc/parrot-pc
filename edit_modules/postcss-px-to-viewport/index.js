...
module.exports = postcss.plugin('postcss-px-to-viewport', function (options) {
  ...
  if (Object.prototype.toString.call(options) === '[object Array]') {
    options.forEach(v => {
      list.push(createOne(v));
    })
  } else {
    list.push(createOne(options));
  }

  return function (css, result) {
    list.forEach(v => {
      var opts = v.opts;
      var pxRegex = v.pxRegex;
      var satisfyPropList = v.satisfyPropList;
      var landscapeRules = v.landscapeRules;
...
function createOne(options) {
  ...
  return {
    pxRegex: getUnitRegexp(opts.unitToConvert),
    satisfyPropList: createPropListMatcher(opts.propList),
    landscapeRules: [],
    opts: opts,
  };
}
